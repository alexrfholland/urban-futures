...
def process_txt_to_df(file_path):
    ...
    # Step 2: Drop the first row (headers in the file)
    data = raw_data[1:]

    # Step 3: Replace all tabs and spaces with commas
    data = [re.sub(r'\s+|\t+', ',', line.strip()) for line in data]

    # Step 4: Write the cleaned data into a CSV-like format in memory
    cleaned_data = StringIO("\n".join(data))

    # Step 5: Define the correct headers (17 columns)
    headers = ['radius (m)', 'length (m)', 
               'start_1', 'start_2', 'start_3', 
               'axis_1', 'axis_2', 'axis_3', 
               'parent', 'extension', 'branch', 'branch_order', 
               'position_in_branch', 'amad', 'SurfCov', 'added', 'UnmodRadius (m)']

    # Step 6: Read the data into a DataFrame, ensuring only 17 columns are kept
    df = pd.read_csv(cleaned_data, delimiter=',', header=None)
    
    # Dropping extra columns if they exist
    df = df.iloc[:, :17]

    # Step 7: Assign the headers to the DataFrame
    df.columns = headers
...
filename_dict = {
    "cylinder_Flagstaff_Large_trees_0_2.txt" : ("large", 7),
    "cylinder_Flagstaff_Large_trees_1_2.txt" : ("large", 8),
    "cylinder_Flagstaff_Large_trees_2_2.txt" : ("large", 9),
    "cylinder_Flagstaff_Large_trees_5_2.txt" : ("large", 10),
    "cylinder_Flagstaff_Large_trees_7_2.txt" : ("large", 11)
...
def process_headers(df):
    # Print initial headers for verification
    print("Initial headers:", df.columns.tolist())
    
    # Modify headers for start_point and axis_direction
    new_columns = []
    for col in df.columns:
        if col == 'start_point':
            new_columns.extend(['start_point1', 'start_point2', 'start_point3'])
        elif col == 'axis_direction':
            new_columns.extend(['axis_direction1', 'axis_direction2', 'axis_direction3'])
        else:
            new_columns.append(col)
    
    # Check the header modification
    print(f"Updated headers: {new_columns}")
    
    # Attempt to split the 'start_point' column into three separate columns if the column exists
    if 'start_point' in df.columns:
        try:
            # Ensure start_point values are split correctly
            df[['start_point1', 'start_point2', 'start_point3']] = pd.DataFrame(df['start_point'].tolist(), index=df.index)
            print(f"Successfully split 'start_point' into: {['start_point1', 'start_point2', 'start_point3']}")
        except Exception as e:
            print(f"Error splitting 'start_point': {e}")
    
    # Attempt to split the 'axis_direction' column into three separate columns if the column exists
    if 'axis_direction' in df.columns:
        try:
            df[['axis_direction1', 'axis_direction2', 'axis_direction3']] = pd.DataFrame(df['axis_direction'].tolist(), index=df.index)
            print(f"Successfully split 'axis_direction' into: {['axis_direction1', 'axis_direction2', 'axis_direction3']}")
        except Exception as e:
            print(f"Error splitting 'axis_direction': {e}")
    
    # Drop original 'start_point' and 'axis_direction' columns
    df.drop(columns=['start_point', 'axis_direction'], inplace=True, errors='ignore')
    print("Dropped original 'start_point' and 'axis_direction' columns.")
    
    # Assign new headers only after confirming the correct number of columns
    if len(df.columns) == len(new_columns):
        df.columns = new_columns
        print("Final headers after processing:", df.columns.tolist())
    else:
        print(f"Length mismatch: DataFrame has {len(df.columns)} columns, but new headers have {len(new_columns)} elements.")
    
    return df

def load_csv_files_and_fix_columns(folder_path):
    """Load all .txt files from a given folder into a list of dataframes using process_txt_to_df."""
    
    # List all .txt files in the folder
    txt_files = [f for f in os.listdir(folder_path) if f.endswith('.txt')]
    
    # Process each .txt file using process_txt_to_df and store the resulting DataFrame
    dataframes = [(file, process_txt_to_df(os.path.join(folder_path, file))) for file in txt_files]
...
def load_csv_files(folder_path):
    ...
    dataframes = [(file, pd.read_csv(os.path.join(folder_path, file), delimiter='\t')) for file in csv_files]
    return dataframes
...
def extract_common_filename_part(filename):
    """Extract the common part of the filename (between the second and fifth underscores)."""
    return '_'.join(filename.split('_')[2:5])

def rename_columns(cylinder_df, graph_df):
    """Rename columns for both cylinder and graph dataframes."""
    
    # Define the mapping for cylinder_df
    cylinder_rename_map = {
        'radius (m)': 'radius',
        'UnmodRadius (m)': 'unmod_radius',
        'length (m)': 'length',
        'branch_order': 'BranchOrder',
        'mad': 'mad',
        'SurfCov': 'surf_cov',
        'position_in_branch': 'PositionInBranch'
    }
    
    # Define the mapping for graph_df
    graph_rename_map = {
        'diameter (m)': 'diameter',
        'volume (L)': 'volume',
        'area (m^2)': 'area',
    ...
    # Print original columns for cylinder_df
    print(f"Original cylinder_df columns: {cylinder_df.columns.tolist()}")
    
    # Rename columns in cylinder_df
    cylinder_df = cylinder_df.rename(columns={k: v for k, v in cylinder_rename_map.items() if k in cylinder_df.columns})
    
    # Print renamed columns for cylinder_df
    print(f"Renamed cylinder_df columns: {cylinder_df.columns.tolist()}")
    
    # Print original columns for graph_df
    print(f"Original graph_df columns: {graph_df.columns.tolist()}")
    
    # Rename columns in graph_df
    graph_df = graph_df.rename(columns={k: v for k, v in graph_rename_map.items() if k in graph_df.columns})
    
    # Print renamed columns for graph_df
    print(f"Renamed graph_df columns: {graph_df.columns.tolist()}")
    
    return cylinder_df, graph_df
...
def transfer_graph_to_cylinders(graph_dfs, cylinder_folder):
    ...
    for graph_filename, graph_df in graph_dfs:
        # Use helper function to extract the common part of the filename
        prefix = extract_common_filename_part(graph_filename)
        graph_dict[prefix] = (graph_filename, graph_df)
    
    # Load cylinder dataframes
    #cylinder_dataframes = load_csv_files(cylinder_folder)
    cylinder_dataframes = load_csv_files_and_fix_columns(cylinder_folder)
    
    # List to hold updated (renamed) dataframes
    updated_cylinder_dataframes = []

    for cylinder_filename, cylinder_df in cylinder_dataframes:
        # Use helper function to extract the common part of the filename
        prefix = extract_common_filename_part(cylinder_filename)
        ...
        if prefix in graph_dict:
            ...
            cylinder_df = process_headers(cylinder_df)

            
            # Rename columns for both dataframes
            cylinder_df, graph_df = rename_columns(cylinder_df, graph_df)
            ...
            print(f'preview of cylinder df: {cylinder_df}')
            print(f'preview of graph df: {graph_df}')
            ...
            # Append the renamed dataframe to the updated list
            updated_cylinder_dataframes.append((cylinder_filename, cylinder_df))
        else:
            print(f"No matching graph file found for cylinder file: {cylinder_filename}")
    
    # Return the updated list of renamed cylinder dataframes
    return updated_cylinder_dataframes
...
input_folder = 'data/revised/lidar scans/elm/treeQSM/large/initial/branch'
output_folder = 'data/revised/lidar scans/elm/treeQSM/large/processed/branch'
...
input_folder_cylinders = 'data/revised/lidar scans/elm/treeQSM/large/initial/cylinder'
output_folder_cylinders = 'data/revised/lidar scans/elm/treeQSM/large/processed/cylinder'
...
first_filename, first_df = cylinder_dfs[0]
print(f'headings of first_df in cylinder_dfs: {first_df.columns.tolist()}')
...
# ELM TEMPLATES
output_folder_templates = 'data/revised/lidar scans/elm/treeQSM/large/4_templates'
output_folder_debug_templates = 'data/revised/lidar scans/elm/treeQSM/large/3_debug'
