...
def resolve_clip_box_transform(spec):
    ...
    if inherited_box is not None:
        ...
        if clip_box_name == CITY_CLIP_BOX_NAME:
            scale_x = max(scale_x, width_x / 2.0 + CITY_CLIP_BOX_XY_PADDING)
            scale_y = max(scale_y, width_y / 2.0 + CITY_CLIP_BOX_XY_PADDING)

    scale_z = max(base_scale_z, height / 2.0 + CITY_CLIP_BOX_Z_PADDING)
