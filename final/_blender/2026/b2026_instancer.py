...
WRITE_RUN_LOG = True
RUN_LOG_DIR = '/Users/alexholland/Coding/volumetric-scenarios-rhino-bim-gia/data/blender/2026/logs'
POLE_BYPASS = True
...
def configure_priority_view_layer_visibility(scene, priority_scenario_name=None):
    ...
    priority_view_layer = scene.view_layers.get('city_priority')
    if priority_view_layer is None:
        return False

    available_scenarios = get_available_scenarios()
    priority_scenario_name = priority_scenario_name or available_scenarios[-1]

    for scenario_name in available_scenarios:
        set_collection_exclude(
            priority_view_layer,
            get_priority_run_collection_name(scenario_name),
            scenario_name != priority_scenario_name,
        )

    print(
        f"Configured 'city_priority' to isolate priority collection for scenario "
        f"'{priority_scenario_name}'"
    )
    return True
...
def configure_scenario_view_layer_visibility(scene):
    ...
    for view_layer in scene.view_layers:
        ...
        main_visible = set(visibility_spec['main'])
        priority_visible = set(visibility_spec['priority'])
        if view_layer.name in SCENARIO_VIEW_LAYER_MAP:
            configured = True
        for candidate in available_scenarios:
            set_collection_exclude(
                view_layer,
                get_run_collection_name(candidate),
                candidate not in main_visible,
            )
            set_collection_exclude(
                view_layer,
                get_priority_run_collection_name(candidate),
                candidate not in priority_visible,
            )
    ...
    if configured:
        print(
            ...
            " city_priority -> positive priority,"
...
def run_scenario(scene, scenario_name):
    ...
    if SITE == 'trimmed-parade':
        df['nodeType'] = 'tree'
        if YEAR == 10:
            df = df[df['structureID'] != 253]
            df = df[df['structureID'] != 255]
            df = df[df['structureID'] != 256]
            df = df[df['structureID'] != 691]
            df = df[df['structureID'] != 696]
    ...
    has_priority_target = SITE == 'city' and scene.view_layers.get('city_priority') is not None
    should_build_priority = has_priority_target and scenario_name == 'positive'
    if SITE == 'city' and not has_priority_target:
        print("\nNo 'city_priority' view layer found; skipping priority tree instance branch")
