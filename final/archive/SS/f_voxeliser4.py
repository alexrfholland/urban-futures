...
def main():
    ...
    for site in sites:
        ...
        ds_updated = update_existing_voxels(ds, dfExisting)

        # Step 3: Append new voxels to the Dataset
        print('Appending new voxels to the Dataset...')
        ds_final = append_new_voxels(ds_updated, dfSparse, voxel_size=voxel_size)

        print('Integration complete!')

        # **Inspect the merged Dataset**
        logger.info(f"Merged Dataset variables: {list(ds_final.data_vars.keys())}")
        logger.info(f"Merged Dataset dimensions: {ds_final.dims}")

        # Print the Dataset summary
        print(ds_final)

        # Ensure that all variables have the same length as 'voxel' dimension
        num_voxels = len(ds_final['voxel'])
        for var in ds_final.data_vars:
            var_length = ds_final[var].sizes['voxel']
        ...
        resource_prefix = 'resource_'
        resource_count_vars = [var for var in ds_final.data_vars if var.startswith(resource_prefix) and var.endswith('_count')]

        # Validate resource counts
        validate_resource_counts(ds_final, voxelised_resource_df, resource_count_vars)

        # Optional: Automated check for total resource counts
        # automated_resource_count_check(ds_final, voxelised_resource_df, resource_count_vars)

        # Print the updated Dataset summary
        print(ds_final)

        # Convert to 3D voxel grid
        print("Converting to 3D voxel grid with dimensions I, J, K...")
        voxelGrid = convert_to_ijk_grid(ds_final, voxel_size=voxel_size)
        logger.info("Converted to 3D voxel grid.")

        # **Inspect the 3D voxel grid**
        logger.info(f"VoxelGrid variables: {list(voxelGrid.data_vars.keys())}")
        logger.info(f"VoxelGrid dimensions: {voxelGrid.dims}")

        # Print the voxelGrid summary
        print(voxelGrid)

        # Visualization and further processing...
        visualize_voxel_dataset(ds_final, site, voxel_size)  # Assuming visualization remains on 'ds_final'

        logger.info(f"Voxelization and resource integration complete for site: {site}")

        # Save ds_final to a NetCDF file
