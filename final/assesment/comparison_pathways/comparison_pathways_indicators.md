# Comparison Pathways Indicators

## Full analysis

The full analysis below provides a technical comparison of the year-180 capability indicators.

**Table 1. Full analysis.**

| Site / Persona | Acquire Resources | Explanation | Communicate | Explanation | Reproduce | Explanation |
|---|---|---|---|---|---|---|
| Parade / Bird | 3.28x peeling bark volume (65.2% vs 19.9%) | Retained ageing elms move into low-control canopy and keep peeling bark. Human-led pruning and replacement still leave some bark, so the gap is moderate. | 1.48x perchable canopy volume (61.0% vs 41.1%) | Both pathways keep substantial branch volume. Nonhuman-led mainly preserves more low-control canopy rather than adding a new perch type. | 11.27x hollow count (92.5% vs 8.2%) | Hollows need retained senescent and snag trees. Human-led replaces most ageing elms before cavities form. |
| Parade / Lizard | 8.06x combined ground-cover/dead branch/epiphyte indicators (72.9% vs 9.0%) | Nonhuman-led retains deadwood and epiphytes and creates more unmanaged ground. Human-led keeps some dead branches but little else. | 1.59x non-paved surface area (81.7% vs 51.4%) | Adjacent parkland means both pathways already retain a large unpaved movement surface. Nonhuman-led mainly adds extra depaved ground. | 439.14x combined nurse-log/fallen-tree indicators (27.5% vs 0.1%) | Nurse logs and fallen trees only accumulate when elms collapse in place. Human-led removal drives the denominator near zero. |
| Parade / Tree | 555.00x senescent biovolume (55.5% vs 0.1%) | Only nonhuman-led leaves elms senescing. Human-led sends over-mature trees to replacement. | 1.83x soil near canopy features (81.3% vs 44.4%) | Adjacent parkland means human-led Parade still has unpaved open ground near some canopy, which allows fungal connections. Nonhuman-led retains many more canopy features, so the indicator still rises. | 710.56x grassland-for-recruitment indicator (59.6% vs 0.1%) | Adjacent parkland has little low-vegetation grassland, a proxy for unmanaged ground, close enough to canopy to count as recruitment space. Nonhuman-led creates much more of that canopy-grassland pairing. |
| Street / Bird | 5.22x peeling bark volume (33.9% vs 6.5%) | Released canopy control and enabled pole structures add bark-bearing habitat. Human-led still keeps some bark on surviving trees. | 2.39x perchable canopy volume (37.9% vs 15.8%) | Enabled poles create an extra perch network, and more trees move to low-control canopy. Human-led relies mostly on managed tree canopy. | 5.84x hollow count (43.1% vs 7.4%) | Artificial hollows on poles plus retained canopy drive the increase. Human-led leaves only a small remnant on high-control trees. |
| Street / Lizard | 14.28x combined ground-cover/dead branch/epiphyte indicators (88.2% vs 6.2%) | Facade and roof habitat plus low-control deadwood expand strongly under nonhuman-led. Human-led barely activates these substrates. | 3.64x non-paved surface area (153.0% vs 42.1%) | Nonhuman-led turns facades and roofs into traversable non-paved surface. Human-led mostly relies on the existing open strip. | 15.00x combined nurse-log/fallen-tree indicators (11.4% vs 0.8%) | Logs, fallen trees and debris are retained across converted edges. Human-led clears most of these refuges. |
| Street / Tree | 5.63x senescent biovolume (12.5% vs 2.2%) | Both pathways keep some senescent trees, but nonhuman-led extends age-in-place and reduces control so more remain senescent. | 3.61x soil near canopy features (62.5% vs 17.3%) | More canopy features sit next to depaved ground under nonhuman-led. Human-led leaves much less canopy-ground overlap. | 12.60x grassland-for-recruitment indicator (44.0% vs 3.5%) | Parking and road-edge conversion creates low-vegetation space near canopy. Human-led leaves only small disconnected fragments. |
| City / Bird | 2.11x peeling bark volume (34.5% vs 16.4%) | Both pathways retain some bark-bearing canopy and structures. Nonhuman-led keeps more trees in low-control ageing states. | 1.47x perchable canopy volume (38.7% vs 26.3%) | Perchable branch volume survives in both pathways, so the gap stays small. Nonhuman-led mainly preserves more canopy. | 2.71x hollow count (54.5% vs 20.1%) | Nonhuman-led accumulates more natural hollows in retained canopy. Human-led still preserves a smaller stock through partial ageing and installed structures. |
| City / Lizard | 4.95x combined ground-cover/dead branch/epiphyte indicators (97.9% vs 19.8%) | Bio-envelope activation is the main driver. Nonhuman-led opens far more facade, green-roof and brown-roof habitat, plus extra deadwood. | 6.72x non-paved surface area (165.0% vs 24.5%) | This metric scales with roof and facade conversion. Human-led enables some surfaces, but much less. | 5.01x combined nurse-log/fallen-tree indicators (27.2% vs 5.4%) | Nonhuman-led enables far more logs and fallen-tree refuges on roofs, facades and ground. Human-led still has some enabled logs. |
| City / Tree | 2.89x senescent biovolume (21.9% vs 7.6%) | Both pathways allow some senescent trees, but nonhuman-led retains about three times more before replacement. | 4.10x soil near canopy features (45.1% vs 11.0%) | This is ground-only, so roofs and facades do not count. The gain comes from more ground-level canopy features and depaved soil. | 6.43x grassland-for-recruitment indicator (51.7% vs 8.0%) | Recruitment also needs ground vegetation close to canopy. Nonhuman-led creates much more of that pairing. |

## Explanation of indicator divergence at Year 180

This summary uses three divergence classes: minimal divergence for values under 2x, moderate divergence for 2x to under 5x, and large divergence for 5x or more, or where the human-led value is effectively zero.

For readability, reported percentages below 0.1% are shown as 0.1%.
...
Hollows depend on support for `Decay` proposals, which allow senescent trees and snag features to persist long enough for hollows to form. Human-led more often rejects these proposals by removing ageing trees before hollows accumulate, so pathway divergence is high.
...
Traversable open ground depends less on proposal support because the adjacent parkland already provides unpaved ground surfaces in the form of lawns and grasslands. Human-led therefore retains a substantial share of this indicator.
...
#### 6.1 Acquire Resources. Large divergence (5.63x, 12.5% vs 2.2%).

Senescent biovolume depends on `Decay` proposals being supported strongly enough to retain ageing trees in place. Human-led rejects most of these proposals, so much less senescent volume persists.
...
Hollows depend on `Decay` proposals being supported strongly enough to retain older canopy long enough for cavities to accumulate. Human-led retains fewer of these features, but young trees in both pathways limit hollow formation.
...
#### 9.1 Acquire Resources. Moderate divergence (2.89x, 21.9% vs 7.6%).

Senescent biovolume depends on `Decay` proposals being supported strongly enough to retain ageing trees before replacement. Human-led allows some of this condition, but much less.
