...
def create_xarray_dataset(centroids, data_dict):
    ...
    Ensures that 'voxel' is the only dimension.
    ...
    Returns:
        xr.Dataset: The created xarray Dataset.
    """
    ...
    logger.info(f"Created xarray Dataset with {len(ds.data_vars)} variables and 'voxel' dimension.")
    return ds
...
def voxelize_polydata(site, voxel_size=1.0):
    ...
    ds = create_xarray_dataset(centroids, data_dict)
...
def visualize_voxel_dataset(ds, plotter=None):
    ...
    for var_name in ds.data_vars:
        if var_name in ['voxel', 'centroid_x', 'centroid_y', 'centroid_z', 'voxel_I', 'voxel_J', 'voxel_K']:
    ...
    # Adjust the scalar name based on available data
...
def main():
    ...
    for site in sites:
        ...
        visualize_voxel_dataset(ds_final)
