...
bl_info = {
    "name" : "Blender Compositor Colormaps",
    "author" : "Jeran Poehls, Modified by Your Name", 
    "description" : "Import Matplotlib color maps into the Compositor",
    "blender" : (3, 0, 0),
    "version" : (1, 0, 0),
    "location" : "Compositor > Tool > Colormaps",
    "warning" : "",
    "doc_url": "", 
    "tracker_url": "", 
    "category" : "Node" 
...
def sna_update_sna_color_libraries_22719(self, context):
    sna_updated_prop = self.sna_compositor_color_libraries  # Fixed property name
...
class SNA_OT_Install_Dependencies_Comp(bpy.types.Operator):
    bl_idname = "sna.install_dependencies_compositor"
...
class SNA_AddonPreferences_Comp(bpy.types.AddonPreferences):
    bl_idname = 'blender_compositor_colormaps'  # Changed from 'blender_compositor_cmaps'
    ...
    def draw(self, context):
        if not (False):
            ...
            if dependencies['sna_deps_installed']:
                ...
                op = col_720D4.operator('sna.install_dependencies_compositor', text='Install Dependencies', icon_value=0, emboss=True, depress=False)




def sna_update_color_map_5D6A6():
    steps = bpy.data.scenes['Scene'].sna_compositor_colormap_steps  # Fixed property name
    cmap_name = bpy.context.scene.sna_compositor_cmaps  # Fixed property name
...
def load_post_handler_197C2(dummy):
    bpy.context.scene.sna_compositor_color_libraries = 'matplotlib'  # Fixed property name


class SNA_PT_COMPOSITOR_COLORMAPS(bpy.types.Panel):
    bl_label = 'Compositor Colormaps'
    bl_idname = 'SNA_PT_COMPOSITOR_COLORMAPS'
    ...
    def draw(self, context):
        ...
        split_FF844.prop(bpy.context.scene, 'sna_compositor_color_libraries', text='', icon_value=0, emboss=True)
        ...
        split_47029.prop(bpy.context.scene, 'sna_compositor_cmaps', text='', icon_value=0, emboss=True)
        ...
        split_20DBF.prop(bpy.context.scene, 'sna_compositor_colormap_steps', text='', icon_value=0, emboss=True)
        op = col_F7EAC.operator('sna.create_compositor_color_ramp', text='Create Compositor Color Ramp', icon_value=0, emboss=True, depress=False)
        if property_exists("bpy.context.active_node.select", globals(), locals()):
            if ((bpy.context.active_node.type == 'COLOR_RAMP') and bpy.context.active_node.select):  # Fixed node type
                op = col_F7EAC.operator('sna.update_compositor_colorramp', text='Update Selected')



class SNA_OT_Update_Compositor_Colorramp(bpy.types.Operator):
    bl_idname = "sna.update_compositor_colorramp"
    bl_label = "Update Compositor Colorramp"
...
class SNA_OT_Create_Compositor_Color_Ramp(bpy.types.Operator):
    bl_idname = "sna.create_compositor_color_ramp"
    bl_label = "Create Compositor Color Ramp"
    ...
    def execute(self, context):
        sna_create_compositor_colorramp()
        return {"FINISHED"}
...
def sna_create_compositor_colorramp():
    steps = bpy.data.scenes['Scene'].sna_compositor_colormap_steps
    cmap_name = bpy.context.scene.sna_compositor_cmaps
    brewer = nodetree['sna_brewer']
    
    # Only handle Compositor context
    if bpy.context.area.ui_type == 'CompositorNodeTree':
        if bpy.context.scene.node_tree:
            nodes = bpy.context.scene.node_tree.nodes
            cramp = nodes.new(type='CompositorNodeColorRamp')
    else:
        return
...
def register():
    ...
    bpy.types.Scene.sna_compositor_cmaps = bpy.props.EnumProperty(name='cmaps', description='', items=sna_cmaps_enum_items)
    bpy.types.Scene.sna_compositor_color_libraries = bpy.props.EnumProperty(name='Color Libraries', description='', items=sna_color_libraries_enum_items, update=sna_update_sna_color_libraries_22719)
    bpy.types.Scene.sna_compositor_colormap_steps = bpy.props.IntProperty(name='Colormap Steps', description='', default=10, subtype='NONE')
    bpy.utils.register_class(SNA_OT_Install_Dependencies_Comp)
    bpy.utils.register_class(SNA_AddonPreferences_Comp)
    bpy.app.handlers.load_post.append(load_post_handler_197C2)
    bpy.utils.register_class(SNA_PT_COMPOSITOR_COLORMAPS)
    bpy.utils.register_class(SNA_OT_Update_Compositor_Colorramp)
    bpy.utils.register_class(SNA_OT_Create_Compositor_Color_Ramp)
...
def unregister():
    ...
    addon_keymaps.clear()
    del bpy.types.Scene.sna_compositor_colormap_steps
    del bpy.types.Scene.sna_compositor_color_libraries
    del bpy.types.Scene.sna_compositor_cmaps
    bpy.utils.unregister_class(SNA_OT_Install_Dependencies_Comp)
    bpy.utils.unregister_class(SNA_AddonPreferences_Comp)
    bpy.app.handlers.load_post.remove(load_post_handler_197C2)
    bpy.utils.unregister_class(SNA_PT_COMPOSITOR_COLORMAPS)
    bpy.utils.unregister_class(SNA_OT_Update_Compositor_Colorramp)
    bpy.utils.unregister_class(SNA_OT_Create_Compositor_Color_Ramp)
