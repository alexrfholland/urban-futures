...
from _futureSim_refactored.blender.bexport import bexport_f_vtk_to_ply_surfaces as f_vtk_to_ply_surfaces
from _futureSim_refactored.paths import scenario_tree_df_path, site_world_reference_vtk_path
...
def get_scenario_polydata(site, voxelSize, resolution, year):
    ...
    filepath = scenario_tree_df_path(site, scenario, year, voxelSize)
...
def get_base_polydata(site):
    sitePath = site_world_reference_vtk_path(site, "site")
    roadPath = site_world_reference_vtk_path(site, "road")
...
resolutions = [0, .25]
filePATH = scenario_tree_df_path(site, scenario, years[0], voxel_size).parent
