...
SHARED_CODE_ROOT = next(parent for parent in Path(__file__).resolve().parents if parent.name == "_futureSim_refactored")
REPO_ROOT = SHARED_CODE_ROOT.parent
if str(SHARED_CODE_ROOT) not in sys.path:
    sys.path.insert(0, str(SHARED_CODE_ROOT))

from _futureSim_refactored.paths import engine_output_nodedf_path, log_mesh_ply_dir, tree_mesh_ply_dir

REPO_DATA_ROOT = REPO_ROOT / "_data-refactored" / "v3engine_outputs"
...
def resolve_tree_ply_folder() -> Path:
    refactored_dir = tree_mesh_ply_dir()
...
def resolve_log_ply_folder() -> Path:
    refactored_dir = log_mesh_ply_dir()
    if refactored_dir.exists():
        return refactored_dir
    for root in iter_existing_bundle_roots():
...
def resolve_feature_csv_path(site: str, scenario: str, year: int) -> Path:
    ...
    for filename in (
        ...
        candidate = engine_output_nodedf_path(asset_site, source_scenario, source_year, 1)
        if candidate.exists():
            return candidate
