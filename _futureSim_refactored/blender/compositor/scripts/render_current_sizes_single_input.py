"""Fast runner for compositor_sizes_single_input.blend.

Single-input runner: one state EXR. Run once per branch
(positive_state, trending_state, positive_priority_state).
...
NAME = "render_current_sizes_single_input"
BLEND_DEFAULT = CANONICAL_ROOT / "compositor_sizes_single_input.blend"
...
def main() -> None:
    ...
    scene = os.environ.get("COMPOSITOR_SCENE_NAME", "SizesSingleInput")
    ...
    config = FastRunnerConfig(
        ...
        file_output_node="SizesSingle::Outputs",
        output_dir=Path(out),
    )
