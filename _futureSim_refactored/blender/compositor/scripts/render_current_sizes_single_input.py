"""Thin runner for single-input size outputs from the canonical blend."""
...
BLEND_PATH = env_path(
    ...
    str(CANONICAL_ROOT / "compositor_sizes_single_input.blend"),
)
EXR_PATH = env_path(
    "COMPOSITOR_SIZES_SINGLE_INPUT_EXR",
    "",
)
OUTPUT_DIR = env_path(
    "COMPOSITOR_SIZES_SINGLE_INPUT_OUTPUT_DIR",
    "",
)
SCENE_NAME = os.environ.get("COMPOSITOR_SCENE_NAME", "SizesSingleInput")


def log(message: str) -> None:
    print(f"[render_current_sizes_single_input] {message}")
...
def main() -> None:
    ...
    if not OUTPUT_DIR:
        raise ValueError("COMPOSITOR_SIZES_SINGLE_INPUT_OUTPUT_DIR is required")
    ...
    output_node = node_tree.nodes.get("SizesSingle::Outputs")
    if output_node is None:
        raise ValueError("Missing 'SizesSingle::Outputs' file output node")
