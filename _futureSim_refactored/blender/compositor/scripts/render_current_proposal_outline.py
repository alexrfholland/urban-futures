"""Thin runner for proposal outlines from the canonical proposal_outline_layers blend.
...
SCENE_NAME = os.environ.get("COMPOSITOR_SCENE_NAME", "ProposalOutline")


def log(message: str) -> None:
    print(f"[render_current_proposal_outline] {message}")
...
def main() -> None:
    ...
    if not OUTPUT_DIR:
        raise ValueError("COMPOSITOR_PROPOSAL_OUTLINE_OUTPUT_DIR is required")
    ...
    output_node = node_tree.nodes.get("ProposalOutlineOutput")
    if output_node is None:
        raise ValueError("Missing 'ProposalOutlineOutput' file output node")
