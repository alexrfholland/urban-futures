"""Thin runner for proposal-only masks from the canonical proposal_only_layers blend.

Opens the canonical template, repaths the EXR input, sets the output directory,
renders, renames outputs, and does NOT save changes back.
"""
...
SCENE_NAME = os.environ.get("COMPOSITOR_SCENE_NAME", "ProposalOnly")


def log(message: str) -> None:
    print(f"[render_current_proposal_only] {message}")
...
def main() -> None:
    ...
    if not OUTPUT_DIR:
        raise ValueError("COMPOSITOR_PROPOSAL_ONLY_OUTPUT_DIR is required")
    ...
    output_node = node_tree.nodes.get("ProposalOnlyOutput")
    if output_node is None:
        raise ValueError("Missing 'ProposalOnlyOutput' file output node")
