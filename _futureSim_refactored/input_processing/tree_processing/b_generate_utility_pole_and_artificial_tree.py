...
CODE_ROOT = REPO_ROOT / "_futureSim_refactored"
BLENDER_EXPORT_DIR = CODE_ROOT / "blender" / "bexport"
...
from _futureSim_refactored.paths import hook_tree_ply_library_dir, tree_mesh_ply_dir, tree_mesh_vtk_dir

def get_snag(filename):
    """Load and return the snag mesh."""
    mesh_folder_directory = tree_mesh_vtk_dir()
    
    # Load the mesh with pyvista
    filename = f'{filename}.vtk'
    mesh = pv.read(str(mesh_folder_directory / filename))
...
if __name__ == '__main__':
    ...
    #export vtk file
    vtk_path = tree_mesh_vtk_dir()
    ply_path = tree_mesh_ply_dir()

    #save clipped snag as artifical tree vtk
    export_path_vtk = vtk_path / f'{artficialTreeName}.vtk'
    clipped_snag.save(str(export_path_vtk))

    ply_filepath = ply_path / f'{artficialTreeName}.ply'

    a_vtk_to_ply.export_polydata_to_ply(clipped_snag, str(ply_filepath))
