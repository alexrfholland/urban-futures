...
from _futureSim_refactored.paths import tree_template_base_root, tree_template_resource_dict_path
###UTILITY FUNCTIONS###
...
def preprocessing_tree_resources(voxelDF, clusterGraph):
    ...
    voxelDF['isTerminal'] = False
    
    # Mark voxels belonging to terminal branches as True
    voxelDF.loc[voxelDF['cluster_id'].isin(terminal_branch_list), 'isTerminal'] = True
    print(f"Marked {voxelDF['isTerminal'].sum()} voxels as terminal branches")
...
def make_senscent_versionB(voxelDF, clusterGraph, seed=42):
...
def make_senscent_version(voxelDF, clusterGraph, seed=42):
    ...
    # Instead of finding leaf nodes, find the child-most valid nodes in senescent communities
    valid_senescent_nodes = []
    for node in clusterGraph.nodes():
        if is_in_senescent_community(node) and clusterGraph.nodes[node].get('isValid', True):
            children = list(clusterGraph.successors(node))
            # Check if this is a child-most valid node
            is_childmost_valid = all(
                not clusterGraph.nodes[child].get('isValid', True) 
                for child in children
            )
            if is_childmost_valid:
                valid_senescent_nodes.append(node)
    
    print(f"\nFound {len(valid_senescent_nodes)} child-most valid nodes in senescent communities")
    
    # Create set to collect terminal nodes
    terminal_nodes = set()

    # Process each valid senescent node and work upwards
    for node in valid_senescent_nodes:
        if node in visited:
            continue
        ...
        # Work up from valid node until we hit a node with radius >= threshold
        while continue_branch and current_node is not None:
            if current_node in visited:
                break
            ...
            if should_prune:
                ...
                if parents and is_in_senescent_community(parents[0]):
                    ...
                    # If no valid parent, this node becomes terminal
                    clusterGraph.nodes[current_node]['isTerminal'] = True
                    terminal_nodes.add(current_node)
                    current_node = None
            else:
                # This node is thick enough - mark it as terminal and stop
                clusterGraph.nodes[current_node]['isTerminal'] = True
                terminal_nodes.add(current_node)
                continue_branch = False

    # Update terminal nodes in DataFrame all at once
    voxelDF.loc[voxelDF['cluster_id'].astype(str).isin(terminal_nodes), 'isTerminal'] = True
    ...
    # Update perch nodes selection to use valid nodes
    perch_nodes = set()
    for node in clusterGraph.nodes():
        if (clusterGraph.nodes[node]['isSenescent'] and 
            clusterGraph.nodes[node].get('isValid', True)):
            # Check if it's a child-most valid node
            children = list(clusterGraph.successors(node))
            is_childmost_valid = all(
                not clusterGraph.nodes[child].get('isValid', True) 
                for child in children
    ...
    # Plot the polydata using isPruned as scalars
    plotter = pv.Plotter()
    plotter.add_mesh(poly, scalars='isTerminal', point_size=10, render_points_as_spheres=True)
    plotter.show()
    plotter.close()"""
    
    return voxelDF, valid_voxelDF

def get_tree_template(allTemplateResourceDF, treeID, voxelDF, clusterGraph):
    """
    allTemplateResourceDF is now a DataFrame with columns:
        - precolonial (bool)
        - size (str)
        - control (str)
        - peeling bark, dead branch, fallen log, leaf litter, hollow, epiphyte (float)
    """
    treeSizes = {
        4 : 'small',
        5 : 'small',
    ...
    improved_mask = (allTemplateResourceDF['precolonial'] == precolonial) & \
                   (allTemplateResourceDF['size'] == treeSizes[treeID]) & \
                   (allTemplateResourceDF['control'] == 'improved-tree')
    
    if improved_mask.any():
        controls.append('improved-tree')


    voxelDF = voxelDF[voxelDF['isValid'] == True]

    # Regular versions for each control type
    for control in controls:
        
        # Get the specific row from the DataFrame
        mask = (allTemplateResourceDF['precolonial'] == precolonial) & \
               (allTemplateResourceDF['size'] == treeSizes[treeID]) & \
               (allTemplateResourceDF['control'] == control)
        
        row = allTemplateResourceDF[mask].iloc[0]
        
        template = assign_resources(voxelDF.copy(), row, seed=treeID)
        ...
        rows.append({
            ...
            'size': treeSizes[treeID], 
            'control': control, 
            'tree_id': treeID, 
            'template': template
        })
        
        # Check if senescing version exists for this combination
        senescing_mask = (allTemplateResourceDF['precolonial'] == precolonial) & \
                        (allTemplateResourceDF['size'] == 'senescing') & \
                        (allTemplateResourceDF['control'] == control)
        
        if senescing_mask.any():
            senescing_row = allTemplateResourceDF[senescing_mask].iloc[0]
            
            senescingTemplate = make_senscent_version(template.copy(), clusterGraph, seed=treeID+1)[1]
            senescingTemplate = assign_hollows_and_epiphytes(
                senescingTemplate, 
                senescing_row['hollow'],
                senescing_row['epiphyte'],
            ...
            senescingTemplate = aa_tree_helper_functions.verify_resources_columns(senescingTemplate)
...
def assign_dead_branches(voxel_df, deadValue, columnName, seed):
    ...
    # Exclude voxels that are fallen logs
    valid_voxels = voxel_df[voxel_df['resource_fallen log'] != 1]
    
    # Step 3: Find the total number of rows (valid voxels)
    total_valid_voxels = valid_voxels.shape[0]
    print(f"Total valid voxels (excluding fallen logs): {total_valid_voxels}")
    ...
    threshold0_groups = valid_voxels.groupby('community_ancestors_threshold0').size()
    ...
    valid_voxels.loc[valid_voxels['community_ancestors_threshold0'] == closest_group_name, columnName] = 1

    # Step 7: Recalculate the number of assigned voxels
    total_voxels_converted = valid_voxels[columnName].sum()
    ...
    if remaining_voxels_needed > 0:
        leiden_groups = valid_voxels.groupby('community-leiden').size()

        # Step 9: Identify valid Leiden groups with isTerminal == True and not part of the community_ancestors_threshold0 group
        valid_leiden_groups = valid_voxels[
            (valid_voxels['isTerminal'] == True) & 
            (valid_voxels['community_ancestors_threshold0'] != closest_group_name)
        ...
        for leiden_group_name, leiden_group_count in valid_leiden_groups.items():
            ...
            if leiden_group_count <= remaining_voxels_needed:
                valid_voxels.loc[valid_voxels['community-leiden'] == leiden_group_name, columnName] = 1
                total_voxels_converted = valid_voxels[columnName].sum()
                remaining_voxels_needed -= leiden_group_count
            else:
                # Use the seed for random sampling
                group_to_add = valid_voxels[
                    (valid_voxels['community-leiden'] == leiden_group_name) & 
                    (valid_voxels[columnName] == 0)
                ].sample(n=int(remaining_voxels_needed), random_state=seed)
                valid_voxels.loc[group_to_add.index, columnName] = 1
                total_voxels_converted = valid_voxels[columnName].sum()
    ...
    column_counts = valid_voxels[columnName].value_counts()
    ...
    if excess_voxels > 0:
        rows_assigned_as_dead = valid_voxels[valid_voxels[columnName] == 1]
        rows_to_unassign = rows_assigned_as_dead.sort_values(by='branch_id').index[:int(excess_voxels)]
        valid_voxels.loc[rows_to_unassign, columnName] = 0
    ...
    # Final error check - compare actual percentage to expected percentage
    actual_percentage = (total_voxels_converted / total_valid_voxels) * 100
    expected_percentage = deadValue
    percentage_difference = abs(actual_percentage - expected_percentage)
    
    if percentage_difference > 5:
        raise ValueError(
            f"Final dead branch percentage ({actual_percentage:.2f}%) differs from target "
            f"({expected_percentage:.2f}%) by more than 5%"
        )
    
    print(f"Final dead branch percentage: {actual_percentage:.2f}%")
    print(f"Target was: {deadValue}%")
    print(f"Percentage difference: {percentage_difference:.2f}%")

    # Final assignments should only be made to non-fallen-log voxels
    voxel_df.loc[valid_voxels.index, columnName] = valid_voxels[columnName]
...
def assign_peeling_bark(voxelDF, peelingBarkValue, columnName, seed):
    ...
    # Exclude voxels that are fallen logs
    valid_voxels = voxelDF[voxelDF['resource_fallen log'] != 1]
    total_valid_voxels = valid_voxels.shape[0]
    ...
    cluster_stats = valid_voxels.groupby('cluster_id').agg({
        'start_radius': 'mean',
    ...
    valid_voxels.loc[valid_voxels['cluster_id'].isin(clusters_to_convert), columnName] = 1
    
    # Calculate how many voxels were converted
    total_converted = valid_voxels[columnName].sum()
    ...
    if excess_voxels > 0:
        ...
        converted_rows = valid_voxels[valid_voxels[columnName] == 1]
    ...
    final_converted = valid_voxels[columnName].sum()
    ...
    value_counts = valid_voxels[columnName].value_counts()
    ...
    # Final assignments should only be made to non-fallen-log voxels
    voxelDF.loc[valid_voxels.index, columnName] = valid_voxels[columnName]
...
def assign_perch_branch(voxelDF, columnName, seed):
    ...
    # Group by cluster_id and find groups with isTerminal == True
    terminal_groups = voxelDF[(voxelDF['isTerminal'] == True) & 
...
def assign_resources(validDF,resourceRow,seed=42):
    validDF['isSenescent'] = False
    # Pass the seed + an offset to each function to ensure different sequences
    print(f'assigning resources for {resourceRow}')

    print(f'resource dict is {resourceRow}')

    validDF = aa_tree_helper_functions.verify_resources_columns(validDF)
    
    validDF = assign_dead_branches(validDF, resourceRow['dead branch'], "resource_dead branch", seed=seed)
    validDF = assign_peeling_bark(validDF, resourceRow['peeling bark'], "resource_peeling bark", seed=seed+1)
    validDF = assign_perch_branch(validDF, "resource_perch branch", seed=seed+2)
    validDF = assign_hollows_and_epiphytes(validDF, resourceRow['hollow'], resourceRow['epiphyte'], "resource_hollow", "resource_epiphyte", seed=seed+3)
    validDF = assign_logs(validDF, resourceRow['fallen log'], "resource_fallen log", resourceRow['control'], resourceRow['size'], seed=seed+4)
...
def mark_small_branches_invalid(voxelDF, clusterGraph, radiusThreshold=0.025):
    """
    Marks branches as invalid if their radius is below threshold, working upwards from leaf nodes.
    Also marks terminal branches as the last valid nodes in each branch path.
    
    Args:
        voxelDF (pd.DataFrame): Voxel dataframe with cluster_id column
        clusterGraph (nx.DiGraph): Directed graph representing tree structure
        radiusThreshold (float): Minimum radius threshold (default 0.025)
    
    Returns:
        tuple: (Updated voxel dataframe, Updated cluster graph)
    """
    def should_invalidate_branch(node):
        """Check if a branch should be invalidated based on radius"""
        radius = float(clusterGraph.nodes[node].get('startRadius', 0.5))
        should_invalidate = radius < radiusThreshold
        return radius, should_invalidate

    # Initialize attributes in graph
    nx.set_node_attributes(clusterGraph, True, 'isValid')
    nx.set_node_attributes(clusterGraph, False, 'isTerminal')
    ...
    leaf_nodes = [n for n in clusterGraph.nodes() if clusterGraph.out_degree(n) == 0]
    print(f"\nFound {len(leaf_nodes)} leaf nodes to check")
    ...
    for leaf in leaf_nodes:
        ...
        # Work up from leaf until we hit a node with radius >= threshold
        while continue_checking and current_node is not None:
            if current_node in visited:
                break
                
            visited.add(current_node)
            radius, should_invalidate = should_invalidate_branch(current_node)
            
            if should_invalidate:
                invalid_clusters.add(current_node)
                clusterGraph.nodes[current_node]['isValid'] = False
                ...
                # This is a valid node - mark it as terminal if it's the first valid node we've found
                if last_valid_node is None:
                    terminal_branches.add(current_node)
                    clusterGraph.nodes[current_node]['isTerminal'] = True
                last_valid_node = current_node
                continue_checking = False

    # Convert sets to lists of integers for DataFrame filtering
    invalid_cluster_list = [int(node) for node in invalid_clusters]
    terminal_branch_list = [int(node) for node in terminal_branches]
    
    print(f"Found {len(invalid_cluster_list)} clusters to mark as invalid")
    print(f"Found {len(terminal_branch_list)} terminal branches")

    # Update DataFrame isValid and isTerminal columns
    invalid_mask = voxelDF['cluster_id'].isin(invalid_cluster_list)
    terminal_mask = voxelDF['cluster_id'].isin(terminal_branch_list)
    
    voxelDF['isValid'] = True  # Initialize all as valid
    voxelDF.loc[invalid_mask, 'isValid'] = False
    
    voxelDF['isTerminal'] = False  # Initialize all as non-terminal
    voxelDF.loc[terminal_mask, 'isTerminal'] = True
    
    # Print statistics
    total_invalid = invalid_mask.sum()
    total_terminal = terminal_mask.sum()
    total_voxels = len(voxelDF)
    
    print(f"Marked {total_invalid} voxels as invalid ({(total_invalid/total_voxels)*100:.2f}% of total)")
    print(f"Marked {total_terminal} voxels as terminal branches ({(total_terminal/total_voxels)*100:.2f}% of total)")
    
    # Print graph statistics
    invalid_nodes = sum(1 for _, data in clusterGraph.nodes(data=True) if not data['isValid'])
    terminal_nodes = sum(1 for _, data in clusterGraph.nodes(data=True) if data['isTerminal'])
    total_nodes = len(clusterGraph)
    
    print(f"Graph summary:")
    print(f"- Invalid nodes: {invalid_nodes} ({(invalid_nodes/total_nodes)*100:.2f}% of total)")
    print(f"- Terminal nodes: {terminal_nodes} ({(terminal_nodes/total_nodes)*100:.2f}% of total)")
...
def main():
    resourceDicFilePath = tree_template_resource_dict_path()
    resourceDic = pd.read_csv(resourceDicFilePath)
    ...
    for idx, (tree_id, filename) in enumerate(fileNameDic.items()):
        ...
        voxelDF, clusterGraph = mark_small_branches_invalid(voxelDF, clusterGraph)
    ...
    input_dir = tree_template_base_root()
