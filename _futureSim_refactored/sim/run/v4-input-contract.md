...
This directory has these sections:
...
| `tree_library_exports/` | Per-tree/log PLY and VTK meshes consumed by bV2 instancers |
| `world/` | World-source PLYs (site + road voxels) consumed by bV2 template setup |
...
## Tree Library Exports (`tree_library_exports/`)

Per-tree and per-log mesh assets used by the bV2 Blender instancers (not by the
sim engine itself). Generated from the tree template library and the log
library.

| Subdir | Format | What it is | Generated by |
|---|---|---|---|
| `treeMeshes/` | VTK | Per-tree surface meshes | `combine_resource_treeMeshGenerator.py` |
| `treeMeshesPly/` | PLY | Per-tree point-cloud PLYs consumed by `resolve_tree_ply_folder()` | tree template export pipeline |
| `treePlyLibrary/` | PLY | Hook-assigned PLYs keyed by control/size/resource | tree template export pipeline |
| `logMeshes/` | VTK | Fallen log surface meshes | `a_log_mesh_generator.py` |
| `logMeshesPly/` | PLY | Log point-cloud PLYs consumed by `resolve_log_ply_folder()` | log export pipeline |

---

## World Sources (`world/`) — optional bV2 preprocessing

World-source PLYs consumed by `bV2_setup_template.py` when building the bV2
template world payload. These are not used by the sim engine itself.

| Subdir | Format | What it is |
|---|---|---|
| `originals/` | PLY | Per-site site + road voxel PLYs (`{site}-siteVoxels.ply`, `{site}-roadVoxels.ply`) |
| `highres/` | PLY | Upsampled road voxel PLYs (4×4 grid at 0.25m), one per site |

### Regenerate highres roads (optional)

If `world/highres/*.ply` is missing, regenerate it from `world/originals/` with:

```bash
uv run python _futureSim_refactored/blender/blenderv2/bv2_helper_high_res_road.py
```

Path constants: `WORLD_INPUTS_ROOT`, `WORLD_ORIGINALS_ROOT`, `WORLD_HIGHRES_ROOT` in `paths.py`.
