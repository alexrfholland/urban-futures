...
from _futureSim_refactored.sim.setup.structure_ids import assign_baseline_tree_structure_ids
...
def generate_baseline(site, voxel_size=1, output_folder=None, visualize=False):
    """
    ...
    output_folder (str | Path | None): Folder to save output files
    ...
    filepath = site_subset_dataset_path(site, voxel_size)
    xarray_dataset = xr.open_dataset(filepath)

    terrain_polydata_path = site_world_reference_vtk_path(site, "road")
    ...
    output_folder = Path(output_folder) if output_folder else scenario_baseline_dir()
    ...
    from _futureSim_refactored.sim.generate_vtk_and_nodeDFs import a_scenario_urban_elements_count
...
def main():
    ...
    parser.add_argument(
        '--output-folder',
        type=str,
        default=str(scenario_baseline_dir(output_mode="canonical")),
