...
All debug outputs are saved to: _statistics-refactored-v3/debug/
"""
...
from _futureSim_refactored.paths import statistics_csv_dir, statistics_debug_dir
...
DEBUG_DIR = statistics_debug_dir()
...
def load_indicator_counts(site, voxel_size=1):
    ...
    Loads from the refactored statistics CSV folder.
    """
    path = statistics_csv_dir() / f'{site}_{voxel_size}_indicator_counts.csv'
    if not path.exists():
...
def load_action_counts(site, voxel_size=1):
    ...
    Loads from the refactored statistics CSV folder.
    """
    path = statistics_csv_dir() / f'{site}_{voxel_size}_action_counts.csv'
    if not path.exists():
        return None
    return pd.read_csv(path)
...
def generate_debug_outputs(site, voxel_size=1, print_summary=True):
    ...
    Outputs are saved to: _statistics-refactored-v3/debug/
