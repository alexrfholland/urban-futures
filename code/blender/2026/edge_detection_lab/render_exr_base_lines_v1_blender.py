...
OUTPUT_DIR = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "outputs" / "exr_city_blender_base_lines_v1"
BLEND_PATH = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "exr_city_blender_base_lines_v1.blend"
...
VARIANT_SPECS = (
    ...
    {
        "name": "base_building_hybrid_ground_clean",
    ...
    {
        "name": "base_building_hybrid_ground_sparse",
        "building_mode": "hybrid",
        "ground_mode": "depth",
        "building_filter": "KIRSCH",
        "ground_filter": "SOBEL",
...
def log(message: str) -> None:
    print(f"[render_exr_base_lines_v1_blender] {message}")
...
def build_building_hybrid_alpha(
    node_tree: bpy.types.NodeTree,
    depth_socket,
    normal_socket,
    ...
    depth_image = masked_scalar_to_image(node_tree, depth_socket, building_mask_socket, f"{prefix}_depthimg", f"{prefix}_depthimg", (-1260.0, y + 120.0))
    normal_bw = rgb_to_bw_node(node_tree, normal_socket, f"{prefix}_normalbw", f"{prefix}_normalbw", (-1260.0, y - 120.0))

    depth_alpha = build_filtered_alpha(
        node_tree,
        depth_image,
        building_mask_socket,
        f"{prefix}_depth",
    ...
    normal_alpha = build_filtered_alpha(
        node_tree,
    ...
    combined = math_node(node_tree, "MAXIMUM", f"{prefix}_hybrid", f"{prefix}_hybrid", (1320.0, y), clamp=True)
    ensure_link(node_tree, depth_alpha, combined.inputs[0])
    ensure_link(node_tree, normal_alpha, combined.inputs[1])
...
def build_scene(scene: bpy.types.Scene) -> list[Path]:
    ...
    for index, spec in enumerate(VARIANT_SPECS):
        y = -220.0 - index * 520.0
        if spec["building_mode"] == "hybrid":
            building_alpha = build_building_hybrid_alpha(
                node_tree,
                exr.outputs["Depth"],
                exr.outputs["Normal"],
