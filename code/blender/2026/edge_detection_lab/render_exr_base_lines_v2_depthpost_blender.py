...
OUTPUT_DIR = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "outputs" / "exr_city_blender_base_lines_v2_depthpost"
BLEND_PATH = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "exr_city_blender_base_lines_v2_depthpost.blend"
...
VARIANT_SPECS = (
    {
        "name": "base_depth_kirsch_punched_clean",
        "building_filter": "KIRSCH",
        "ground_filter": "SOBEL",
        "building_curve_low": 0.02,
        "building_curve_high": 0.24,
        "ground_curve_low": 0.04,
        "ground_curve_high": 0.20,
        "building_low": 0.03,
        "building_high": 0.10,
        "ground_low": 0.04,
        "ground_high": 0.11,
    ...
    {
        "name": "base_depth_kirsch_punched_all",
        "building_filter": "KIRSCH",
        "ground_filter": "KIRSCH",
        "building_curve_low": 0.02,
        "building_curve_high": 0.22,
        "ground_curve_low": 0.04,
        "ground_curve_high": 0.22,
        "building_low": 0.028,
        "building_high": 0.088,
        "ground_low": 0.035,
        "ground_high": 0.095,
    ...
    {
        "name": "base_depth_kirsch_punched_sparse",
        "building_filter": "KIRSCH",
        "ground_filter": "KIRSCH",
        "building_curve_low": 0.03,
        "building_curve_high": 0.28,
        "ground_curve_low": 0.05,
        "ground_curve_high": 0.26,
        "building_low": 0.05,
        "building_high": 0.13,
        "ground_low": 0.06,
        "ground_high": 0.15,
...
def log(message: str) -> None:
    print(f"[render_exr_base_lines_v2_depthpost_blender] {message}")
...
def contrast_ramp_image(
    ...
    node.color_ramp.elements[1].position = high
    node.color_ramp.elements[1].color = (1.0, 1.0, 1.0, 1.0)
    ensure_link(node_tree, image_socket, node.inputs["Fac"])
    return node
...
def build_scene(scene: bpy.types.Scene) -> list[Path]:
    ...
    depth_base_normalized = normalize_node(node_tree, depth_base_image, "base_depth_normalized", "base_depth_normalized", (-1240.0, -160.0))
    ...
    for index, spec in enumerate(VARIANT_SPECS):
        ...
        building_contrast = contrast_ramp_image(
            node_tree,
            depth_base_normalized.outputs[0],
            f"{spec['name']}_building_contrast",
            f"{spec['name']}_building_contrast",
            (-980.0, y),
            spec["building_curve_low"],
            spec["building_curve_high"],
        )
        building_alpha = build_filtered_alpha(
            node_tree,
            building_contrast.outputs["Image"],
            building_mask.outputs["Alpha"],
            f"{spec['name']}_building",
            y,
            spec["building_filter"],
            spec["building_low"],
            spec["building_high"],
            spec["building_core"],
            spec["building_wide"],
        ...
        ground_contrast = contrast_ramp_image(
            node_tree,
            depth_base_normalized.outputs[0],
            f"{spec['name']}_ground_contrast",
            f"{spec['name']}_ground_contrast",
            (-980.0, y - 220.0),
            spec["ground_curve_low"],
            spec["ground_curve_high"],
        )

        ground_alpha = build_filtered_alpha(
            node_tree,
            ground_contrast.outputs["Image"],
