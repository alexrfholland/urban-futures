...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "outputs" / "exr_city_blender_baseline_refresh_20260326" / "ao"
DEFAULT_BLEND_PATH = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "exr_city_blender_baseline_refresh_ao.blend"
...
def log(message: str) -> None:
    print(f"[render_exr_ao_baseline_blender] {message}")
...
def build_scene(scene: bpy.types.Scene) -> list[Path]:
    ...
    pathway_ao = set_alpha_node(node_tree, pathway.outputs["AO"], mask_pathway.outputs["Alpha"], "AO Pathway Trees", "AO Pathway Trees", (-900.0, 500.0))
    priority_ao = set_alpha_node(node_tree, priority.outputs["AO"], mask_visible_priority.outputs["Value"], "AO Priority Trees", "AO Priority Trees", (-900.0, 120.0))
    existing_ao = set_alpha_node(node_tree, existing.outputs["AO"], existing.outputs["Alpha"], "AO Existing Full", "AO Existing Full", (-900.0, -260.0))

    rendered_paths = [
        output_node(node_tree, pathway_ao.outputs["Image"], "pathway_ao_masked", (-520.0, 500.0)),
        output_node(node_tree, priority_ao.outputs["Image"], "priority_ao_masked", (-520.0, 120.0)),
        output_node(node_tree, existing_ao.outputs["Image"], "existing_condition_ao_full", (-520.0, -260.0)),
    ]
