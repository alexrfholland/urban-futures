...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "outputs" / "exr_city_blender_baseline_refresh_20260326" / "resources"
DEFAULT_BLEND_PATH = REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "exr_city_blender_baseline_refresh_resources.blend"
...
def log(message: str) -> None:
    print(f"[render_exr_arboreal_resource_fills_baseline_blender] {message}")
...
def math_node(node_tree, operation: str, name: str, label: str, location, clamp=True):
...
def solid_color_image(node_tree, source_socket, name: str, label: str, location, rgba):
...
def alpha_over_node(node_tree, name: str, label: str, location, bottom_socket, top_socket):
...
def hex_to_linear_rgba(hex_value: str):
...
def socket_by_name(node, preferred_name: str):
    ...
    for socket in node.outputs:
        if socket.name.lower().replace(" ", "_") == normalized_preferred:
            return socket
    raise KeyError(f"Socket '{preferred_name}' not found on {node.name}.")


def multiply_sockets(node_tree, left_socket, right_socket, name: str, label: str, location):
...
def build_scene_layer(node_tree, slug: str, exr_node, source_image_socket, resource_base_mask_socket, rendered_paths: list[Path], y: float):
    colored_outputs = {}
    ...
    for index, resource in enumerate(RESOURCE_SPECS):
        ...
        rendered_paths.append(output_node(node_tree, rgba.outputs["Image"], f"{slug}_{resource['slug']}", (180.0, resource_y)))
    ...
    rendered_paths.append(output_node(node_tree, combined_socket, f"{slug}_resource_combined", (180.0 + len(stack_bottom_to_top) * 240.0, combined_y)))
...
def main() -> None:
    ...
    pathway = image_node(node_tree, PATHWAY_EXR, "EXR Pathway", "EXR Pathway", (-1420.0, 620.0))
    priority = image_node(node_tree, PRIORITY_EXR, "EXR Priority", "EXR Priority", (-1420.0, -620.0))

    mask_visible_pathway = id_mask_node(node_tree, pathway.outputs["IndexOB"], "mask_visible-arboreal_pathway", "mask_visible-arboreal_pathway", (-1100.0, 500.0))
    mask_all_priority = id_mask_node(node_tree, priority.outputs["IndexOB"], "mask_all-arboreal_priority", "mask_all-arboreal_priority", (-1100.0, -760.0))
    mask_visible_priority = math_node(node_tree, "MULTIPLY", "mask_visible-arboreal_priority", "mask_visible-arboreal_priority", (-880.0, -620.0))
    ...
    build_scene_layer(node_tree, "pathway", pathway, pathway.outputs["Image"], mask_visible_pathway.outputs["Alpha"], rendered_paths, 620.0)
    build_scene_layer(node_tree, "priority", priority, priority.outputs["Image"], mask_visible_priority.outputs["Value"], rendered_paths, -620.0)

    composite = new_node(node_tree, "CompositorNodeComposite", "Composite", "Composite", (1820.0, 20.0))
