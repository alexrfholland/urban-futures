...
OUTPUT_DIR = Path(
    os.environ.get(
        "EDGE_LAB_PROPOSAL_ONLY_OUTPUT_DIR",
        "/Users/alexholland/Coding/volumetric-scenarios-rhino-bim-gia/data/blender/2026/edge_detection_lab/outputs/proposal_only_layers",
    )
).expanduser()
BLEND_PATH = Path(
    os.environ.get(
        "EDGE_LAB_PROPOSAL_ONLY_BLEND",
        "/Users/alexholland/Coding/volumetric-scenarios-rhino-bim-gia/data/blender/2026/edge_detection_lab/proposal_only_layers.blend",
    )
).expanduser()

RED = (1.0, 0.0, 0.0, 1.0)
PROPOSAL_CHANNELS = (
    ("proposal-release-control", "proposal-only_release-control"),
    ("proposal-decay", "proposal-only_decay"),
    ("proposal-recruit", "proposal-only_recruit"),
    ("proposal-colonise", "proposal-only_colonise"),
    ("proposal-deploy-structure", "proposal-only_deploy-structure"),
)
...
def build_red_mask(node_tree, value_socket, slug: str, x: float, y: float):
    threshold = new_node(
        node_tree,
        "CompositorNodeMath",
        f"{slug}::threshold",
    ...
    rgb = new_node(node_tree, "CompositorNodeRGB", f"{slug}::rgb", "rgb", (x + 320.0, y))
    rgb.outputs[0].default_value = RED

    rgba = new_node(node_tree, "CompositorNodeSetAlpha", f"{slug}::rgba", "rgba", (x + 640.0, y))
    rgba.mode = "REPLACE_ALPHA"
    ensure_link(node_tree, rgb.outputs[0], rgba.inputs["Image"])
    ensure_link(node_tree, threshold.outputs["Value"], rgba.inputs["Alpha"])
...
def main() -> None:
    ...
    for row, (channel_name, slug) in enumerate(PROPOSAL_CHANNELS):
        y = 360.0 - row * 280.0
        image_socket = build_red_mask(node_tree, socket_by_name(exr_node, channel_name), slug, 240.0, y)
        rendered.append((slug, image_socket))

    composite = new_node(node_tree, "CompositorNodeComposite", "Composite", "Composite", (1380.0, 280.0))
    viewer = new_node(node_tree, "CompositorNodeViewer", "Viewer", "Viewer", (1380.0, 140.0))
