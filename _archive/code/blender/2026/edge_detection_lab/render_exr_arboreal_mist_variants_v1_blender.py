...
OUTPUT_ROOT = Path(
    os.environ.get(
        "EDGE_LAB_OUTPUT_DIR",
        str(REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "outputs" / "exr_city_blender_0000_arboreal_mist_v1"),
    )
)
BLEND_PATH = Path(
    os.environ.get(
        "EDGE_LAB_BLEND_PATH",
        str(REPO_ROOT / "data" / "blender" / "2026" / "edge_detection_lab" / "exr_city_blender_0000_arboreal_mist_v1.blend"),
    )
)
...
def log(message: str) -> None:
    print(f"[render_exr_arboreal_mist_variants_v1_blender] {message}")
...
def build_variant_stage(scene: bpy.types.Scene, scene_name: str, spec: dict, visible_path: Path, mist_visible_path: Path, output_dir: Path) -> dict[str, str]:
    ...
    visible = image_node(node_tree, visible_path, f"{scene_name}_visible_png", f"{scene_name}_visible_png", (-1400.0, 220.0))
    mist_visible = image_node(node_tree, mist_visible_path, f"{scene_name}_mist_visible_png", f"{scene_name}_mist_visible_png", (-1400.0, -60.0))
    mist_bw = rgb_to_bw_node(node_tree, mist_visible.outputs["Image"], f"{scene_name}_mist_bw", f"{scene_name}_mist_bw", (-1140.0, -60.0))
    mist_rgba = separate_rgba_node(node_tree, mist_visible.outputs["Image"], f"{scene_name}_mist_rgba", f"{scene_name}_mist_rgba", (-1140.0, -260.0))
    ...
    edge_alpha = build_edge_width(node_tree, edge_normalized.outputs[0], mist_rgba.outputs["A"], variant_name, spec, -60.0)
...
def main() -> None:
    ...
    summary = {
        "workflow_id": "blender_exr_arboreal_mist_v1",
