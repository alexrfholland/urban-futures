...
def create_line_vtk(gdf):
    ...
    for index, row in gdf.iterrows():
        ...
        if isinstance(geom, LineString):
            ...
            print(f"Unsupported geometry type for index {index}: {type(geom)}")
            continue
        ...
        for line_index, line in enumerate(lines):
            ...
            if coords.shape[1] == 2:
                coords = np.column_stack((coords, np.full(len(coords), z)))
            
            linePointSec = coords.tolist()

            nPoints = len(linePointSec)
            ...
            lineTubes[f"{index}_{line_index}"] = partialLineUgrid
