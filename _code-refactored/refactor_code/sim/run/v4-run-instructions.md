...
**All commands must be run from the repo root** (the directory containing `_code-refactored/`). The `uv run python` commands use paths relative to that root.
...
Full indicator definitions: `_code-refactored/refactor_code/sim/v4_indicator_definitions.md`
...
for site in trimmed-parade city uni; do
  for scenario in positive trending; do
    for year in 0 1 10 30 60 90 120 150 180; do
      uv run python _code-refactored/refactor_code/outputs/report/render_proposal_v4.py \
...
When `REFACTOR_RUN_OUTPUT_ROOT` is not set, `refactor_code.paths.refactor_run_output_root()` falls back to the last logged output root. To inspect recent runs:
