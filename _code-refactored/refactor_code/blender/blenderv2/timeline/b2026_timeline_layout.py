...
SHARED_CODE_ROOT = next(parent for parent in Path(__file__).resolve().parents if parent.name == "_code-refactored")
REPO_ROOT = SHARED_CODE_ROOT.parent
if str(SHARED_CODE_ROOT) not in sys.path:
    sys.path.insert(0, str(SHARED_CODE_ROOT))

from refactor_code.paths import engine_output_nodedf_path, log_mesh_ply_dir, tree_mesh_ply_dir
