"""Experiment v7 — high threshold without boost on mist.

Just Kirsch → hard threshold, no boost. Sweep thresholds from very low
to see where tree edges appear and where canopy mess drops off.

Variants:
  v1_t001   — threshold 0.001
  v2_t005   — threshold 0.005
  v3_t01    — threshold 0.01
  v4_t02    — threshold 0.02
  v5_t03    — threshold 0.03
  v6_t05    — threshold 0.05
  v7_t08    — threshold 0.08
  v8_t10    — threshold 0.10

Run:
  /Applications/Blender.app/Contents/MacOS/Blender --background --python \\
    _code-refactored/refactor_code/blender/compositor/scripts/experiment_mist_highthreshold_v7.py
"""
...
OUTPUT_ROOT = REPO_ROOT / "_data-refactored" / "compositor" / "outputs" / "experiment_mist_threshold_only_20260408"
...
def build_variant(nt, mist_socket, mask_socket, prefix, y, threshold):
    """Mist → Kirsch → hard threshold → BW → mask → colour. No boost."""
    x = -1200
...
def main():
    ...
    blend_path = OUTPUT_ROOT / "experiment_mist_threshold_only.blend"
    try:
