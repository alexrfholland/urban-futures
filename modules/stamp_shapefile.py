...
if __name__ == "__main__":
    ...
    SITE = 'city'
    SITEPATH = f'data/{SITE}/flattened-{SITE}.vtk'
    siteData = pv.read(SITEPATH)

    filepath = f'outputs/deployables/{SITE}/deployables-{SITE}.shp'
    ...
    read_and_plot(SITE, siteData, SHAPEFILE_PATHS, BOUNDING_BOX_SIZE,deleteAttributes=True)
    ...
    SHAPEFILE_PATHS = [
        'data/deployables/shapefiles/parkingmedianB/parking_median_buffer.shp'
    ]


    #read_and_plot(SITE, siteData, SHAPEFILE_PATHS, BOUNDING_BOX_SIZE,deleteAttributes=True,buffer=0)
    read_and_plot(SITE, siteData, SHAPEFILE_PATHS ,deleteAttributes=True,buffer=0)


    #print('hello')
    #print(siteData.point_data.get('HA'))

    # Initialize the PyVista plotter
    plotter = pv.Plotter()
    plotter.add_mesh(siteData, scalars = 'parkingmedianB-isparkingmedianB', cmap = 'rainbow', point_size=5.0, render_points_as_spheres=True)
    plotter.show()
