...
def classify_points_poly(polydata, search, _returnIndex=True, unclassified='unclassified'):
    ...
    for attr in polydata.point_data.keys():
        ...
        if len(array.shape) == 1:
            # Single-dimensional attribute, add directly
            data[attr] = array
        else:
            # Multi-dimensional attribute, use NumPy to create a list of tuples
    ...
    sampled_df.to_csv('data/testdataframe.csv', index=False)

    
    classifications = classify_points(df, search, unclassified, returnIndex=_returnIndex)
...
if __name__ == "__main__":
    ...
    classified_array = classify_points(dataframe, search_criteria, returnIndex=True)
