...
- run instructions: `_futureSim_refactored/sim/run/v4-run-instructions.md`
- indicator definitions: `_futureSim_refactored/sim/v4_indicator_definitions.md`
- indicator extraction script: `_futureSim_refactored/sim/v4_indicator_extract.py`
- indicator comparisons format: `_futureSim_refactored/sim/v4 comparisons.md`
- visualisation instructions: `_futureSim_refactored/v4visualisation-instructions.md`
...
All commands must be run from `_futureSim_refactored/` (or prefixed with `cd _futureSim_refactored &&`) so that `_futureSim_refactored` is importable.
...
See `_futureSim_refactored/sim/run/v4-run-instructions.md` for full step-by-step commands.
...
The four spatial recruitment mechanisms map to full or partial recruit interventions:
...
| `under-canopy-linked` | `rewild-larger-patch` | Full | `scenario_underCanopyLinkedRecruitZone` |
| `ground` | `rewild-larger-patch` | Full | `scenario_rewildGroundRecruitZone` |

`under-canopy-linked` is the upgraded form of `under-canopy`: when >50% of a footprint-depaved parent tree's canopy voxels (via `node_CanopyID`) overlap the ground-recruit zone (`sim_Turns <= sim_TurnsThreshold`), the parent is relabeled `footprint-depaved-connected` and its canopy voxels form a separate zone with full (reserve) support.
...
Per-site/scenario parameters are in `_futureSim_refactored/sim/setup/params_v3.py`.
...
1. **Zone computation** — `calculate_under_node_treatment_status()` computes four spatial zone masks on the possibility-space dataset (see Recruitment below for detail on each zone).
2. **Allocation** — `apply_recruit()` places saplings in each zone. Full support (`rewild-larger-patch`) for node-rewild, under-canopy-linked, and ground zones; partial support (`rewild-smaller-patch`) for under-canopy zones.
...
- **Full support** recruit intervention (`rewild-larger-patch`) — saplings generated in larger rewilded patches (node-rewild, under-canopy-linked, and ground recruit mechanisms) are assigned the **reserve** rate, reflecting increased thriving in larger patches.
- **Partial support** recruit intervention (`rewild-smaller-patch`) — saplings generated under isolated individual tree canopies (under-canopy recruit mechanism) keep the **urban** rate, reflecting smaller patches where urban conditions persist.
...
The engine supports four distinct recruitment mechanisms, each with its own zone mask, allocation logic, and mortality rate:
...
| `under-canopy` | `scenario_underCanopyRecruitZone` | Canopy voxels mapped via `node_CanopyID` to isolated footprint-depaved/exoskeleton tree NodeIDs | Urban (0.06) |
| `under-canopy-linked` | `scenario_underCanopyLinkedRecruitZone` | Canopy voxels of footprint-depaved parents whose canopy >50% overlaps the ground-recruit zone (relabeled `footprint-depaved-connected`) | Reserve (0.03) |
...
- **Node-rewild / under-canopy / under-canopy-linked**: Primary method is voxel-mask placement (random sample from zone voxels with spacing check). Fallback is parent-offset placement (offset from parent tree position by `BUFFER_RECRUIT_PARENT_OFFSET_METERS`).
...
V4 indicators use the `acquire`/`communicate`/`reproduce` capability naming. Full definitions in `_futureSim_refactored/sim/v4_indicator_definitions.md`.
...
Script: `_futureSim_refactored/outputs/report/render_proposal_v4.py`
...
Script: `_futureSim_refactored/outputs/report/render_debug_recruit.py`
...
Full instructions: `_futureSim_refactored/v4visualisation-instructions.md`

## Run Log

Persistent CSV at `_data-refactored/run_log.csv` (columns: `timestamp`, `name`, `output_root`, `description`).

Appended by the batch runner on every run. When `REFACTOR_RUN_OUTPUT_ROOT` is not set, `_futureSim_refactored.paths.refactor_run_output_root()` falls back to the last logged output root.
...
All paths are relative to `_futureSim_refactored/`.
...
- V3 had two recruit types (`buffer-feature`, `rewild-ground`). V4 has four (`node-rewild`, `under-canopy`, `under-canopy-linked`, `ground`) with specific `recruit_mechanism` values.
- Four spatial zone masks (`scenario_nodeRewildRecruitZone`, `scenario_underCanopyRecruitZone`, `scenario_underCanopyLinkedRecruitZone`, `scenario_rewildGroundRecruitZone`) computed centrally in `calculate_under_node_treatment_status()`.
